...
class interactive_plotting:
	...
	def plotting(self):
		...
		for attr_id, i in zip(self.attribute_ids, range(len(self.attribute_ids))):
			...
			save_all_flexDPE_button.on_click(lambda identity = self.attribute_ids[i], radio = radio_group: self.write_all_to_flexPDE(identity, radio))
	...
	def estimate_RSF(self, attrname, radio):
		ion_pot = np.loadtxt("/home/solli/Documents/Prosjektoppgave/RSF_estimation/ionization_potentials.txt")
